...
#include "NullImplementation/NullScaleformHelper.h"
...
void CSystem::InitScaleformSystem(const SSystemInitParams& startupParams)
{
	...
	// Try to find a Scaleform4 selector module interface
	size_t numFactories = 1;
	ICryFactory* pSelectorFactory = nullptr;
	GetCryFactoryRegistry()->IterateFactories(cryiidof<Cry::ScaleformModule::ISelectorModule>(), &pSelectorFactory, numFactories);
	if (numFactories == 1 && pSelectorFactory)
	{
		// Initialize Scaleform4 via a selector module
		std::shared_ptr<Cry::ScaleformModule::ISelectorModule> pModule =
			cryinterface_cast<Cry::ScaleformModule::ISelectorModule, ICryUnknown>(pSelectorFactory->CreateClassInstance());

		if (pModule)
		{
			pModule->LoadScaleformImplementation(m_env, startupParams);
		}
	}

	if (!m_env.pScaleformHelper)
	{
		// No Scaleform4 has been initialized, try to initialize Scaleform3
		if (!InitializeEngineModule(startupParams, DLL_SCALEFORM3, cryiidof<IScaleformHelperEngineModule>(), false))
		{
			m_env.pScaleformHelper = nullptr;
			CryLog("Attempt to load Scaleform3 library from '%s' failed, this feature will not be available", DLL_SCALEFORM3);
		}
	}

	if (!m_env.pScaleformHelper || !m_env.pScaleformHelper->Init())
	{
		if (m_env.pScaleformHelper)
		{
		...
		CryLog("Unable to initialize Scaleform library, this feature will not be available");
	}
	else
	{
	...
	if (!m_env.pScaleformHelper)
		m_env.pScaleformHelper = new CNullScaleformHelper(!gEnv->IsDedicated());
}
...
#define NSIGHT_DLL_PATH L"C:/Program Files/NVIDIA Corporation/Nsight Graphics 2023.3.2/SDKs/NsightGraphicsSDK/0.8.0/lib/x64/NGFX_Injection.dll"

void NSightSetup()
{
	HMODULE nsightModule = LoadLibraryW(NSIGHT_DLL_PATH);
	typedef NGFX_Injection_Result(*pNGFX_Injection_InjectToProcess)(const NGFX_Injection_InstallationInfo* pInstallation, const NGFX_Injection_Activity* pActivity);
	typedef NGFX_Injection_Result(*pNGFX_Injection_EnumerateInstallations)(uint32_t* pCount, NGFX_Injection_InstallationInfo* pInstallations);
	typedef NGFX_Injection_Result(*pNGFX_Injection_EnumerateActivities)(const NGFX_Injection_InstallationInfo* pInstallation, uint32_t* pCount, NGFX_Injection_Activity* pActivities);
	if (nsightModule)
	{
		pNGFX_Injection_InjectToProcess nsightInject = (pNGFX_Injection_InjectToProcess)GetProcAddress(nsightModule, "NGFX_Injection_InjectToProcess");
		pNGFX_Injection_EnumerateInstallations nsightEnumerateInstallations = (pNGFX_Injection_EnumerateInstallations)GetProcAddress(nsightModule, "NGFX_Injection_EnumerateInstallations");
		pNGFX_Injection_EnumerateActivities nsightEnumerateActivities = (pNGFX_Injection_EnumerateActivities)GetProcAddress(nsightModule, "NGFX_Injection_EnumerateActivities");
		uint32_t countInstallations = 0;
		NGFX_Injection_InstallationInfo ngfx_Injection_InstallationInfo[8];
		NGFX_Injection_Result result = nsightEnumerateInstallations(&countInstallations, ngfx_Injection_InstallationInfo);
		assert(result == NGFX_INJECTION_RESULT_OK);
		if (countInstallations > 0)
		{
			uint32_t countActivities;
			NGFX_Injection_Activity ngfx_Injection_Activity[8];
			result = nsightEnumerateActivities(ngfx_Injection_InstallationInfo, &countActivities, ngfx_Injection_Activity);
			assert(result == NGFX_INJECTION_RESULT_OK);
			for (uint32 index = 0; index < countActivities; index++)
			{
				if (ngfx_Injection_Activity[index].type == NGFX_Injection_ActivityType::NGFX_INJECTION_ACTIVITY_FRAME_DEBUGGER)
				{
					result = nsightInject(&ngfx_Injection_InstallationInfo[0], &ngfx_Injection_Activity[index]);
					assert(result == NGFX_INJECTION_RESULT_OK);
				}
			}
		}
